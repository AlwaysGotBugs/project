...
<h3 class="mb-4">تعديل الخبر: {{ $news->title }}</h3>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                    ...
                    @foreach ($types as $value => $label)
                        <option value="{{ $value }}" {{ old('type', $news->type) == $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @error('type')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            {{-- <div class="mb-3">
                ...
                @if ($news->image)
                    <div class="mt-2">
                        ...
                        <img src="{{ Storage::url($news->image) }}" alt="Current Image" style="max-width: 200px; height: auto; border-radius: 4px;">
                    </div>
                @endif
            </div> --}}
